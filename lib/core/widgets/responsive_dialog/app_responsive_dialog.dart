...
class AppResponsiveDialog extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    if (type == DialogType.fullScreen) {
      return Material(
        child: SafeArea(
          child: Container(color: backgroundColor(context), child: buildContent(context)),
        ),
      );
    }
    ...
    if (type == DialogType.dialog) {
      return Dialog(
        ...
        child: ClipRRect(
          ...
          child: Container(color: backgroundColor(context), constraints: constraints, child: buildContent(context)),
        ),
      );
    }
    if (type == DialogType.overlay) {
      return Container(
        ...
        decoration: BoxDecoration(
          ...
          boxShadow: const [BoxShadow(color: Color(0x43323c2b), offset: Offset(0, 3), blurRadius: 25)],
          borderRadius: BorderRadius.circular(12),
        ),
  ...
  Widget buildContent(BuildContext context) => Column(
    ...
    children: [
      Column(
        ...
        children: [
          if (type == DialogType.fullScreen && (smallTitle != null || onBackPressed != null)) ...[smallHeader(context)],
          if (header != null) ...[
            if (type != DialogType.dialog) ...[
              AppDialogHeader(icon: header!.$1, title: header!.$2, subtitle: header!.$3, iconColor: iconColor),
            ],
            if (type == DialogType.dialog) ...[
      ...
      children: [
        if (onBackPressed != null)
          Align(
            ...
            child: AppCloseButton(onPressed: onBackPressed!),
          ),
        if (smallTitle != null) Align(child: Text(smallTitle!, style: context.headlineSmall)),
      ],
    ),
  );
